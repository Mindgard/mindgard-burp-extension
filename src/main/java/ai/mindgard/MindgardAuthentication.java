...
public class MindgardAuthentication {
    ...
    private final MindgardSettingsUI settings;

    public MindgardAuthentication(MindgardSettingsUI settings) {
    ...
    public MindgardAuthentication(MindgardSettingsUI settings, File tokenFile, HttpClient http, Function<String,HttpRequest.BodyPublisher> bodyPublisherFactory, DeviceCodeFlow.Factory deviceCodeFlow, Runnable sleep) {
    ...
    public String auth() {
        try (Stream<String> lines = Files.lines(tokenFile.toPath())) {
            ...
            var request = HttpRequest.newBuilder()
                    .uri(URI.create(Constants.DOMAIN + "/oauth/token"))
